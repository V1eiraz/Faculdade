...
void espaço(int c, int l, char campo[GRU][GRU])
{ // colando espaço ' ' em toda a matriz
    for (int i = 0; i < l; i++)
    {
        for (int j = 0; j < c; j++)
        {
...
void info(int r, int l, int l1, int c, int c1, char campo[GRU][GRU]) // preenchendo a matriz com as informações
{
    ...
    for (int i = 0; i < human; i++)
    {
        l1 = rand() % l;
        c1 = rand() % c;
        if (campo[l1][c1] == ' ') // se estiver vazio ele vai alocar H
        {
            campo[l1][c1] = 'H';
        }
        else
    ...
    int zona;
    zona = 1;
    for (int i = 0; i < zona; i++) // se estiver vazio ele vai alocar Z
    {
        l1 = rand() % l;
        c1 = rand() % c;
        if (campo[l1][c1] == ' ')
    ...
    for (int i = 0; i < r; i++) // se estiver vazio ele vai alocar R
    {
        l1 = rand() % l;
        c1 = rand() % c;
        if (campo[l1][c1] == ' ')
...
void fuga_humana(int pilhax[BBB], int pilhay[BBB], int topo, int c, int l, char campo[GRU][GRU], int x, int y) // fazendo a função do caminho
{
    ...
    topo = 0;         // impilhando a posição inicial
    ...
    int px = 0;
    int py = 0;
    while (topo >= 0) // enquanto tiver lugar para explorar, ele vai ir
    {
        x = pilhax[topo]; // x e y vão receber a ultima posição
        y = pilhay[topo];
        topo--; // desempilha para testar as direções
        if (campo[x][y] == ' ')
        {
        ...
        for (i = 0; i < 4; i++) // conferindo as direções
        {
            ...
            if (px >= 0 && px < l && py >= 0 && py < c && visitado[px][py] == 0) // conferindo se ja passou por la e os limites da matriz
            {
                ...
                {
                    topo++;
                    pilhax[topo] = px; // a pilha vai receber a ultima posição
                    pilhay[topo] = py;
                    visitado[px][py] = 1;
                }
            }
    ...
    else // se mesmo passando por isso tudo, ele não chegar na zona é porque não tinha caminhos possiveis
    {
